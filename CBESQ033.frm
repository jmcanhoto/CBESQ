...
Begin VB.Form frmCAIFGestaoInscricoes 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "CAIF - Gestão de Inscrições"
   ClientHeight    =   6600
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8700
   ...
   Begin VB.Label lblTexto 
      AutoSize        =   -1  'True
      Caption         =   "Seleccionar Inscrições da Valência"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmCAIFGestaoInscricoes"
...
Private Function tAbreBD()
    ...
    Set mWSInscricoes_Idosos = DBEngine.CreateWorkspace("GestaoInscricoes_Idosos", gUtilizador.Nome, gUtilizador.Password)
...
TrataErro:
    tRetorno = False
    Call ErrosGerais("CAIF - Gestão de Inscrições-Abrir BD", Err.Number, Err.Description)
    Resume SaiDaFuncao
...
Private Sub cmdAlterar_Click()
    ...
    If sgrdGestaoInscricoes.Rows <> 0 Then
        ...
        Set fFichaInscricaoCAIF = New frmCAIFFichaInscricao
        ' Carrega a Variavel para passar o Botao de onde vai
        cBotaoOrigem = "Altera"
        ' faz o load da form
        fFichaInscricaoCAIF.Show
    End If
End Sub
...
On Error GoTo TrataErro
            ...
            ' apagar Inscrição
            Set qryApagarInscricao = mBDInscricoes_Idosos.QueryDefs("CAIF INSCRICOES Apagar")
...
TrataErro:
    ...
    Call ErrosGerais("CAIF - Apagar Inscrição", Err.Number, Err.Description)
...
Private Sub cmdPassar_Click()
    ...
    If sgrdGestaoInscricoes.Rows <> 0 Then
        ...
        If iRespMsgBox = vbYes Then
            ...
            Set qryPassaUtente = mBDInscricoes_Idosos.QueryDefs("CAIF INSCRICOES Passa a Utente")
            ' parametros de input
            ...
            Set qryPassaUtente = mBDInscricoes_Idosos.QueryDefs("CAIF INSCRICOES Copia para CAIF UTENTES")
            ' parametros de input
            qryPassaUtente.Parameters("Num_Utente") = lNovoNumUtenteCAIF
            qryPassaUtente.Parameters("Num_Inscricao") = sgrdGestaoInscricoes.Columns(0).Value
            qryPassaUtente.Parameters("Utiliz") = gUtilizador.Nome
            ' executa a query
            qryPassaUtente.Execute dbFailOnError
...
TrataErro:
    mWSInscricoes_Idosos.Rollback
    Call ErrosGerais("CAIF - Passa a Utente", Err.Number, Err.Description)
    Resume SairDoProcedimento
...
Private Sub CarregaGridInscricoes(ByRef Grid As SSDBGrid, ByVal iOrdem)
    ...
    End Select
    ' abre a tabela
    Set recTabelaINSCRICOES = mBDInscricoes_Idosos.OpenRecordset(cSql, dbOpenDynaset, dbReadOnly)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmCAIFGestaoInscricoes = Nothing
End Sub
