...
Begin VB.Form frmCAIFGestaoRecibos 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "CAIF - Gestao de Recibos"
   ClientHeight    =   6720
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8610
   ...
   End
End
Attribute VB_Name = "frmCAIFGestaoRecibos"
...
Dim mWSRecibos_Idosos As Workspace
Dim mBDRecibos_Idosos As Database
...
Private Sub CarregaGridUtentes(ByRef Grid As SSDBGrid, ByVal iOrdem)
    ...
    End Select
    ' abre a tabela
    Set recTabelaUTENTES = mBDRecibos_Idosos.OpenRecordset(cSql, dbOpenSnapshot, dbReadOnly)
...
On Error GoTo TrataErro
    Set mWSRecibos_Idosos = DBEngine.CreateWorkspace("Recibos_Idosos", gUtilizador.Nome, gUtilizador.Password)
    Set mBDRecibos_Idosos = mWSRecibos_Idosos.OpenDatabase(cBD_Path & cNomeBD)
...
On Error GoTo TrataErro
        ...
        cSql = cSql & "WHERE (((RECIBOS_IDOSOS.NUM_UTENTE)=" & sgrdGestaoUtentes.Columns(0).Value & ") " & _
            ...
            "ORDER BY RECIBOS_IDOSOS.ANO ASC, RECIBOS_IDOSOS.COD_MES ASC,RECIBOS_IDOSOS.NUM_RECIBO ASC;"
    
        ' insere o registo em Temp32
        Set recRECIBOS = mBDRecibos_Idosos.OpenRecordset(cSql, dbOpenSnapshot)
...
'                .Destination = crptToWindow
            ...
            If iRespMsgBox = vbYes Then
                Set qryPagaRecibo = mBDRecibos_Idosos.QueryDefs("CAIF RECIBOS Pagar")
                ' parametros de input
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    For Each mBD In mWSRecibos_Idosos.Databases
