...
Begin VB.Form frmDeclaracoesIRS 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Declarações para o I. R. S."
   ClientHeight    =   4725
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5925
   ...
   End
End
Attribute VB_Name = "frmDeclaracoesIRS"
...
'    cAno = "2007"
    ...
    cNomeMapa = "CBESQ013.RPT"

On Error GoTo TrataErro

    ' apaga os registos da Temp32
    cSql = "DELETE * FROM LISTA_DECLARACOES;"
    ' apaga o registo em Temp32
    mBDDeclaracoesTemp.Execute cSql, dbFailOnError
    
    ' carrega a variavela com o Sql
    cSql = "INSERT INTO LISTA_DECLARACOES (NUM_UTENTE,COD_INST,COD_SALA,NOME," & _
        ...
        "UTENTES.NOME,UTENTES.NOME_ENC_EDU,UTENTES.NUM_CONTRIBUINTE,'" & Trim$(cAno) & "'," & _
        "Sum(RECIBOS.TOTAL_MENSALIDADE) AS QUANTIA "
    cSql = cSql & "FROM UTENTES INNER JOIN RECIBOS ON UTENTES.NUM_UTENTE = RECIBOS.NUM_UTENTE " & _
        "WHERE (((Year([RECIBOS]![DATA_PAG]))=" & CInt(cAno) & ")) "
    ...
    cSql = cSql & " GROUP BY UTENTES.NUM_UTENTE,UTENTES.COD_INST,UTENTES.COD_SALA," & _
        "UTENTES.NOME, UTENTES.NOME_ENC_EDU,UTENTES.NUM_CONTRIBUINTE;"
    ...
    Set recDeclaracoes = mBDDeclaracoesTemp.OpenRecordset("LISTA_DECLARACOES", dbOpenDynaset)
    ...
    With rptDeclaracoes
        ...
        Else
            MsgBox "Não foi encontrado o Mapa!", vbInformation, "Impressão da Declaração para o I.R.S."
...
TrataErro:
    Call ErrosGerais("Declarações para o I.R.S.", Err.Number, Err.Description)
...
Private Sub cboNumUtente_DropDown()
    ' carrega a combo
    Call CarregacboNumUtentesTodos(cboNumUtente, cboInstituicao.Text, cboSalas.Text)
End Sub
