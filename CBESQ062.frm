...
Begin VB.Form frmCAIFAlterarRecibosCriados 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "CAIF - Gestao de Recibos de Utentes (Fechados)"
   ClientHeight    =   9660
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8775
   ...
   End
End
Attribute VB_Name = "frmCAIFAlterarRecibosCriados"
...
Private Sub CarregaGridRecibos(ByRef Grid As SSDBGrid, ByVal iOrdem)
    Dim recTabelaRECIBOS_IDOSOS As Recordset
    Dim cLinha
    
    Grid.Redraw = False
    ' começa a construir o Sql
    cSql = "SELECT NUM_UTENTE,NOME,NUM_RECIBO FROM RECIBOS_IDOSOS WHERE (ESTADO_REC='P' OR ESTADO_REC='D')"
    ...
    ' abre a tabela
    Set recTabelaRECIBOS_IDOSOS = mBDAlteraRecibosCriados.OpenRecordset(cSql, dbOpenSnapshot, dbReadOnly)
    ' carrega o data control da grid
    Set datRecibos.Recordset = recTabelaRECIBOS_IDOSOS
...
Private Function tAbreBD()
    ...
    Set mWSAlteraRecibosCriados = DBEngine.CreateWorkspace("CAIF Alteracoes", gUtilizador.Nome, gUtilizador.Password)
...
On Error GoTo TrataErro
        ...
        Set qryAlteraRecibo = mBDAlteraRecibosCriados.QueryDefs("CAIF RECIBOS Altera Recibo Criado")
...
Private Sub CarregaGridAlteracoes(ByRef Grid As SSDBGrid)
    ...
    With Grid
        ...
        cSql = "SELECT * FROM RECIBOS_IDOSOS WHERE NUM_RECIBO=" & sgrdGestaoRecibos.Columns(2).Value
        ...
        If Not (recALTERACOES.EOF And recALTERACOES.BOF) Then
            ...
            .AddItem "Multa por Atraso" & vbTab & recALTERACOES!VALOR2
            .AddItem "Despesas de Saúde" & vbTab & recALTERACOES!VALOR3
            .AddItem "Artigos de Higiene" & vbTab & recALTERACOES!VALOR4
