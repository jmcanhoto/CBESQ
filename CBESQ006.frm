...
Begin VB.Form frmGestaoUtentes 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Gestao de Utentes"
   ClientHeight    =   6405
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8625
   ...
   End
End
Attribute VB_Name = "frmGestaoUtentes"
...
Dim mWSUtentes As Workspace
Dim mBDUtentes As Database
...
Private Sub CarregaGridUtentes(ByRef Grid As SSDBGrid, ByVal iOrdem)
    ...
    End Select
    ' abre a tabela
    Set recTabelaUTENTES = mBDUtentes.OpenRecordset(cSql, dbOpenSnapshot, dbReadOnly)
...
On Error GoTo TrataErro
    Set mWSUtentes = DBEngine.CreateWorkspace("Utentes", gUtilizador.Nome, gUtilizador.Password)
    Set mBDUtentes = mWSUtentes.OpenDatabase(cBD_Path & cNomeBD)
    
    GoTo SaiDaFuncao

TrataErro:
    tRetorno = False
    Call ErrosGerais("Gestão de Utentes-Abrir BD", Err.Number, Err.Description)
    Resume SaiDaFuncao
...
Private Sub cmdAlterar_Click()
    ...
    If sgrdGestaoUtentes.Rows <> 0 Then
        ...
        Set fFichaUtente = New frmFichaUtente
...
On Error GoTo TrataErro
        ...
        Set recRECIBOS = mBDUtentes.OpenRecordset(cSql, dbOpenSnapshot)
...
On Error GoTo TrataErro
        ...
        cSql = "SELECT RECIBOS.ANO,RECIBOS.COD_MES,TABMESES.NOME,RECIBOS.COD_INST,TABINSTITUICAO.NOME,RECIBOS.COD_SALA,TABSALAS.NOME," & _
            "RECIBOS.NOME,RECIBOS.MENSALIDADE,RECIBOS.TOTAL_MENSALIDADE,RECIBOS.VALOR1," & _
            "RECIBOS.VALOR2,RECIBOS.VALOR3,RECIBOS.VALOR4,RECIBOS.VALOR5,RECIBOS.NUM_RECIBO,RECIBOS.MENSALIDADE_PCTG,RECIBOS.TOTAL_MENSALIDADE "
        cSql = cSql & "FROM ((RECIBOS INNER JOIN TABMESES ON " & _
            "RECIBOS.COD_MES = TABMESES.COD_MES) INNER JOIN TABINSTITUICAO ON " & _
            "RECIBOS.COD_INST = TABINSTITUICAO.COD_INST) INNER JOIN TABSALAS ON " & _
            "(RECIBOS.COD_SALA = TABSALAS.COD_SALA) AND (RECIBOS.COD_INST = TABSALAS.COD_INST) "
...
'                .Destination = crptToWindow
            ...
            If iRespMsgbox = vbYes Then
                Set qryPagaRecibo = mBDUtentes.QueryDefs("RECIBOS Pagar")
                ' parametros de input
...
Private Sub cmdSaida_Click()
    ...
    If sgrdGestaoUtentes.Rows <> 0 Then
        ...
        If iRespMsgbox = vbYes Then
            ...
            End If
On Error GoTo TrataErro
            mWSUtentes.BeginTrans
            ' dar saida de utente
            Set qrySaidaUtente = mBDUtentes.QueryDefs("UTENTES Data Saida")
            ...
            Set qrySaidaUtente = mBDUtentes.QueryDefs("UTENTES Data Saida RECIBOS")
...
TrataErro:
    mWSUtentes.Rollback
    Call ErrosGerais("Saida de Utente", Err.Number, Err.Description)
    Resume SairDoProcedimento
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    For Each mBD In mWSUtentes.Databases
