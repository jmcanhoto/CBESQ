...
Begin VB.Form frmProlongamento 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Prolongamento de Horário"
   ClientHeight    =   5880
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5910
   ...
   Begin VB.CommandButton cmdNovo 
      ...
      Width           =   1200
   End
   Begin VB.Data datProlongamento 
      Caption         =   "datProlongamento"
      Connect         =   "Access 2000;"
      DatabaseName    =   ""
      ...
      Width           =   2535
   End
   Begin SSDataWidgets_B.SSDBGrid sgrdProlongamento 
      Bindings        =   "CBESQ022.frx":0000
      Height          =   2355
      Left            =   150
      TabIndex        =   5
      Top             =   2520
   ...
   End
End
Attribute VB_Name = "frmProlongamento"
...
Dim mWSProlongamento As Workspace
Dim mBDProlongamento As Database
...
Private Function tAbreBD()
    ...
    Set mWSProlongamento = DBEngine.CreateWorkspace("Prolongamento", gUtilizador.Nome, gUtilizador.Password)
    Set mBDProlongamento = mWSProlongamento.OpenDatabase(cBD_Path & cNomeBD)

    GoTo SaiDaFuncao

TrataErro:
    tRetorno = False
    Call ErrosGerais("Prolongamento de Horário-Abrir BD", Err.Number, Err.Description)
    Resume SaiDaFuncao
...
Private Sub cboNome_Click()
    ...
    Call CarregaGridProlongamento(sgrdProlongamento, cboNome.Columns(1).Value)
End Sub
...
'    If Trim$(cboNumFunc.Text) <> vbNullString Then
        ...
        Call CarregaGridProlongamento(sgrdProlongamento, cboNumFunc.Columns(0).Value)
'    End If
End Sub
...
Private Sub cmdAlterar_Click()
    ...
    If sgrdProlongamento.Rows <> 0 Then
        ' cria nova instancia da Form
        Set fFichaProlHora = New frmFichaProlHora
...
Private Sub cmdApagar_Click()
    ...
    Dim qryApagarProl As QueryDef
    Dim iRespMsgBox
    Set mProcessamento = New Processamento
    ' se houver registos Edita o que estiver selecionado
    If sgrdProlongamento.Rows <> 0 Then
        ' confirma se quer mesmo apagar
        iRespMsgBox = MsgBox("Confirme se quer apagar o Prolongamento ?", vbYesNo, "Apagar Prolongamento")
        If iRespMsgBox = vbYes Then
On Error GoTo TrataErro
            mWSProlongamento.BeginTrans
            ' apagar Inscrição
            Set qryApagarProl = mBDProlongamento.QueryDefs("PROLONGAMENTO Apagar")
    
            qryApagarProl.Parameters("Contador") = CLng(sgrdProlongamento.Columns(5).Value)

            ' executa a query
            qryApagarProl.Execute dbFailOnError
            mWSProlongamento.CommitTrans
On Error GoTo 0
            ' faz o refresh
            datProlongamento.Refresh
           sgrdProlongamento.Refresh
        End If
    End If
    GoTo SairDoProcedimento
    
TrataErro:
    mWSProlongamento.Rollback
    Call ErrosGerais("Apagar Prolongamento", Err.Number, Err.Description)
...
Private Sub cmdNovo_Click()
    If Trim$(cboNumFunc.Text) = vbNullString Then
        MsgBox "Tem de seleccionar um Funcionário!", vbInformation, "Prolongamento"
        Exit Sub
    End If
    ' cria nova instancia da Form
    Set fFichaProlHora = New frmFichaProlHora
...
Private Sub CarregaGridProlongamento(ByRef Grid As SSDBGrid, ByVal lNumFunc)
    ...
    cSql = "SELECT DATA_PROLG,HORA_INI & ':' & MIN_INI AS INICIO," & _
                "HORA_FIM & ':' & MIN_FIM AS FIM," & _
                " MIN_TOTAL AS TOTAL,MOTIVO_PROLG,CONTADOR FROM PROLONGAMENTO"
    
    cSql = cSql & " WHERE NUM_FUNCIONARIO=" & lNumFunc & " ORDER BY DATA_PROLG ASC"
   
    Set recPROLONGAMENTO = mBDProlongamento.OpenRecordset(cSql, dbOpenSnapshot, dbReadOnly)
    
    Set datProlongamento.Recordset = recPROLONGAMENTO
...
Private Sub Form_Load()
    ...
    Call CarregaGridProlongamento(sgrdProlongamento, 0)
End Sub

Private Sub Form_Unload(Cancel As Integer)
    Dim mBD As Database, mTab As Recordset
    For Each mBD In mWSProlongamento.Databases
        For Each mTab In mBD.Recordsets
            mTab.Close
        Next
        mBD.Close
    Next
    Set mWSProlongamento = Nothing
    fFrmMDIPrincipal.atbOpcoes.Tools("mnuJan").Menu.Tools.Remove Me.Caption
    Set frmProlongamento = Nothing
End Sub


Private Sub sgrdProlongamento_InitColumnProps()
    With sgrdProlongamento
        ...
        .AllowUpdate = False
        .BackColorOdd = dCorAmarelo
        .Caption = "Lista de Prolongamentos"
        .Font.Name = "MS Sans Serif"
        .Font.Size = 10
        .ForeColorEven = &H0&
        .FieldSeparator = vbTab
